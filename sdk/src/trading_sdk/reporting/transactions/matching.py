...
def match_transactions(
  ...
  for op in events:
    if (matches := matcher.match(op.expected_flows)) is None:
      raise ValueError(f'Could not match the flows for the operation: {op}')
    used.update(matches)
    transactions.append(Transaction(
      operation=op,
      flows=[flows[i] for i in matches] + list(op.fixed_flows)
    ))
