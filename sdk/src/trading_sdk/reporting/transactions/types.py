...
class Trade(BaseEvent[D], Generic[D]):
  type: Literal['trade'] = field(default='trade', init=False)
  base: str
  quote: str
  qty: Decimal
  """Base asset quantity."""
...
class FutureTrade(BaseEvent[D], Generic[D]):
  type: Literal['future_trade'] = field(default='future_trade', init=False)
  instrument: str
  size: Decimal
  price: Decimal
  liquidity: Literal['maker', 'taker']
...
class SingleEvent(BaseEvent[D], Generic[D]):
  ...
  def of(cls, id: str, flow: Flow[D2]) -> 'Event[D2]':
    ...
    return cls(id=id, time=flow.time, asset=flow.asset, qty=flow.change, type=flow.label, details=flow.details)


@dataclass(kw_only=True, frozen=True)
class FeeEvent(SingleEvent[D], Generic[D]):
  type: Literal['fee'] = field(default='fee', init=False)

@dataclass(kw_only=True, frozen=True)
class InternalTransfer(SingleEvent[D], Generic[D]):
  type: Literal['internal_transfer'] = field(default='internal_transfer', init=False)
...
class Bonus(SingleEvent[D], Generic[D]):
  type: Literal['bonus'] = field(default='bonus', init=False)

@dataclass(kw_only=True, frozen=True)
class Funding(SingleEvent[D], Generic[D]):
  type: Literal['funding'] = field(default='funding', init=False)

@dataclass(kw_only=True, frozen=True)
class Settlement(SingleEvent[D], Generic[D]):
  type: Literal['settlement'] = field(default='settlement', init=False)

@dataclass(kw_only=True, frozen=True)
class CryptoDeposit(SingleEvent[D], Generic[D]):
  """Deposit from a blockchain."""
  type: Literal['crypto_deposit'] = field(default='crypto_deposit', init=False)
  network: str
  tx_hash: str
  idx: int | None = None
...
class FiatDeposit(SingleEvent[D], Generic[D]):
  type: Literal['fiat_deposit'] = field(default='fiat_deposit', init=False)
  method: str
  fiat_currency: str
...
class CryptoWithdrawal(Withdrawal[D], Generic[D]):
  ...
  type: Literal['crypto_withdrawal'] = field(default='crypto_withdrawal', init=False)
  network: str
  tx_hash: str
  idx: int | None = None
  """Index of the event within the same transaction."""
...
class FiatWithdrawal(Withdrawal[D], Generic[D]):
  type: Literal['fiat_withdrawal'] = field(default='fiat_withdrawal', init=False)
  method: str
  fiat_currency: str
...
class EthereumTransaction(BaseEthereumTransaction[D], Generic[D]):
  type: Literal['ethereum_transfer'] = field(default='ethereum_transfer', init=False)
  value: Decimal
  """Value sent [ETH]"""
...
class ERC20Transfer(BaseEthereumTransaction[D], Generic[D]):
  ...
  type: Literal['erc20_transfer'] = field(default='erc20_transfer', init=False)
  fee: Decimal | None = None
  """Fee paid [ETH]"""
...
class Transaction(Generic[D]):
  ...
  @classmethod
  def single(cls, id: str, flow: Flow[D2]) -> 'Transaction[D2]':
    event = SingleEvent.of(id, flow)
    return cls(event=event, flows=[flow] + list(event.fixed_flows))