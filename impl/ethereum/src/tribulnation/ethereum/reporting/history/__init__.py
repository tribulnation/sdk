...
class EthereumHistory(_History):
  ...
  def etherscan(
    ...
    return EtherscanHistory(
      address=address, chain_id=CHAIN_IDS[network], node=node,
      rpc_url=rpc_url, etherscan=etherscan,
    )
  ...
  def moralis(
    ...
    return MoralisHistory(
      address=address, chain=MORALIS_CHAINS[network], node=node,
      rpc_url=rpc_url, moralis=moralis,
    )
