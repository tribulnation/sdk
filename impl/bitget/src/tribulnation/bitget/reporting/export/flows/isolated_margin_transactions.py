...
from tribulnation.sdk.reporting.history import Flow
import tribulnation.sdk.util.csv as util

class isolated_margin_transactions:
  """Parsing Bitget's isolated margin transactions export.

  Data must be downloaded as a CSV file from:

  > Bitget > Data Export > Margin > Isolated Margin Transactions
  ...
  **Expected schema:**
  ...
  def parse(path: str, tz: timezone, *_, **__):
    df = isolated_margin_transactions.load(path, tz)
    yield from isolated_margin_transactions.parse_df(df)
  ...
  def parse_df(df: pd.DataFrame):
    for _, row in df.iterrows():
      ...
      yield Flow(
        ...
        raw=row.to_dict(),
        source='export:isolated_margin_transactions',
      )
