...
from tribulnation.sdk.reporting.history import SpotTrade
import tribulnation.sdk.util.csv as util

class cross_margin_order_history:
  """Parsing Bitget's cross margin order history export.

  Data must be downloaded as a CSV file from:

  > Bitget > Data Export > Margin > Cross Margin Order History
  ...
  **Expected schema:**
  ...
  def load(path: str, tz: timezone) -> pd.DataFrame:
    ...
    util.validate_schema(df, cross_margin_order_history.schema, id=path)
  ...
  def parse(path: str, tz: timezone, *_, **__):
    df = cross_margin_order_history.load(path, tz)
    yield from cross_margin_order_history.parse_df(df)
  ...
  def parse_df(df: pd.DataFrame):
    for _, row in df.iterrows():
      ...
      yield SpotTrade(
        ...
        fee_asset=None,
        raw=row.to_dict(),
        source='export:cross_margin_order_history',
      )
