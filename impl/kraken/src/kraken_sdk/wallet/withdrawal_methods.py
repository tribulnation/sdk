...
class WithdrawalMethods(SdkMixin, _WithdrawalMethods):
    @SDK.method
    async def fetch_withdraw_methods(self) -> dict:
        """Fetch withdraw methods from Kraken. Returns raw API response."""
        return await self.client.private_post_withdrawmethods()
    ...
    async def withdrawal_methods(
        ...
        *,
        ...
        r = await self.fetch_withdraw_methods()
        if r.get("error"):
