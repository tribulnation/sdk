...
validators so the parsers see exactly the `TypedDict`s a live call hands them.
`sdk-dev test report mexc` verifies the live surface separately with a scoped key.
"""

from typing_extensions import Any, AsyncIterator, Sequence, cast
...
def test_windows_slice_a_wide_span_into_inclusive_capital_windows():
  """A 30-day request needs five seven-day windows, not one 90-day request."""
  start = datetime(2025, 1, 1, tzinfo=UTC)
  end = start + timedelta(days=30)
  slices = list(windows(start, end, history.CAPITAL_WINDOW))
  assert len(slices) == 5
  assert slices[0] == (start, start + timedelta(days=7))
  assert all(upper - lower <= timedelta(days=7) for lower, upper in slices)
...
async def test_history_defaults_and_discovers_symbols(monkeypatch: pytest.MonkeyPatch):
  """No bounds or user market list are needed, even for assets no longer held."""
  ...
  client = SimpleNamespace(
    spot=SimpleNamespace(
      http=SimpleNamespace(
        market=SimpleNamespace(exchange_info=exchange_info),
      )
    )
  )
  calls: list[tuple[object, ...]] = []

  async def source(*args: object) -> AsyncIterator[HistoryRecord]:
    """Capture orchestration without making account-history requests."""
    calls.append(args)
    return
    yield  # pragma: no cover

  for name in ('spot_trades', 'deposits', 'withdrawals', 'funding'):
    monkeypatch.setattr(history, name, source)
  assert [row async for row in history.history(report(client))] == []
  assert len(calls) == 4
  assert calls[0][1] == ['BTCUSDT', 'ETHUSDT']
  lower, upper = calls[0][-2:]
  assert isinstance(lower, datetime) and isinstance(upper, datetime)
  assert upper - lower == timedelta(days=30)


@pytest.mark.parametrize('source', ['deposits', 'withdrawals'])
async def test_capital_sweeps_use_accepted_weekly_queries(source: str):
  """Both capital endpoints receive legal windows even if intermediate pages are empty."""
  endpoint = AsyncMock(return_value=[])
  wallet = SimpleNamespace(deposit_history=endpoint, withdraw_history=endpoint)
  client = SimpleNamespace(spot=SimpleNamespace(http=SimpleNamespace(wallet=wallet)))
  start = datetime(2026, 8, 1, tzinfo=UTC)
  end = start + timedelta(days=30)
  sweep = history.deposits if source == 'deposits' else history.withdrawals
  assert [row async for row in sweep(report(client), start, end)] == []
  assert endpoint.await_count == 5
  windows_seen: list[tuple[datetime, datetime]] = [
    (call.kwargs['start_time'], call.kwargs['end_time'])
    for call in endpoint.await_args_list
  ]
  assert windows_seen[0][0] == start and windows_seen[-1][1] == end
  assert all(upper - lower <= timedelta(days=7) for lower, upper in windows_seen)
  assert all(
    previous[1] + timedelta(milliseconds=1) == following[0]
    for previous, following in zip(windows_seen, windows_seen[1:])
  )
