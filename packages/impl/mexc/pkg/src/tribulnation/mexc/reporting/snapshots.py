...
class Snapshots(Mixin, _Snapshots):
  ...
  async def futures_balances(self) -> Balances:
    ...
    return Balances(
      {
        b.get('currency') or '': Decimal(str(b.get('availableBalance') or '0'))
        + Decimal(str(b.get('positionMargin') or '0'))
        for b in data
      }
    )
  ...
  async def futures_positions(self):
    ...
    for pos in data:
      ...
      if contract is None:
        raise ValueError(
          f'MEXC contract info response did not include data for {symbol}'
        )
      ...
      out[symbol].append(
        Position(size=size, avg_price=Decimal(str(pos.get('openAvgPrice') or '0')))
      )
