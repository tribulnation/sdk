...
Four sources feed it: spot fills for the configured symbols, crypto deposits, crypto
withdrawals and futures funding settlements. MEXC unifies none of them: `myTrades` is
per symbol, the two capital ledgers are account-wide but capped at 90 days per query,
...
"""Widest window the deposit and withdrawal history endpoints accept."""
...
async def spot_trades(
  ...
  """Yield one record per spot fill in the window, for every configured symbol."""
  for symbol in symbols:
    ...
    for lower, upper in windows(start, end, TRADES_WINDOW):
      ...
      for t in fills:
        ...
        yield record(trade, id=f'spot-trade-{trade.id}')
...
async def history(
  ...
  `start` is required: no MEXC source serves all time, and each stops at a different
  horizon (a month for fills, none documented for the capital ledgers), so an
  open-ended window would return a silently truncated result rather than everything.

  Raises:
    ValueError: `start` is `None`.
  """
  if start is None:
    raise ValueError('MEXC history requires a start.')
  end = end or datetime.now(timezone.utc)
  async for r in spot_trades(self, spot_markets, start, end):
    yield r
