# tribulnation-kucoin 0.2.0

## SDK 2 compatibility

1. Requires tribulnation-sdk >=2.0.0 and the Typed dependency floors declared in this package.
2. Private reporting is spot-side only and sweeps all discovered spot symbols. Trade history now paginates the requested interval once per symbol, avoiding repeated sweeps outside upstream retention. API history remains best-effort. Public-market expansion remains deferred; no Futures access is required or promised.
...
3. Merging release/kucoin into main triggers publication of tribulnation-kucoin 0.2.0. No merge or publication has occurred during PR preparation.
...
1. Mainnet Wallet, Earn and Report checks pass, including the complete all-symbol history sweep. The run used a direct connection because the private API rejected the rotating proxy.
2. Sanitized local results, content fingerprints and published dependency pins are committed under [release-evidence/kucoin](https://github.com/tribulnation/sdk/tree/release/sdk/release-evidence/kucoin). All 12 reports independently verify in a second clean environment.
