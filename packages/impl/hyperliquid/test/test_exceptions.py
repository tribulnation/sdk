"""Tests for Hyperliquid exception translation, particularly HTTP 429 -> RateLimited."""
...
from tribulnation.hyperliquid.core.exc import wrap_exceptions
from tribulnation.sdk.core import ApiError, Context, RateLimited, SDK


def _underlying_error(status: int, body: str | None = 'null') -> core.ApiError:
  """Build the shape `typed_hyperliquid`'s HTTP transport actually raises: a bare
  `ApiError(status_code, response_text)` for any non-200 response -- the live report
  suite observed this exact shape, `ApiError(429, null)`, from a testnet rate limit.
  """
  return core.ApiError(status, body)
...
async def test_wrap_exceptions_keeps_other_api_errors_generic() -> None:
  source = _underlying_error(503, 'Service Unavailable')
