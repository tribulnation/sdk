...
async def funding_rates(
  self: PerpMarketMixin, start: datetime | None = None, end: datetime | None = None
  ...
  async for chunk in self.client.info.funding_history_paged(
    ...
    yield [
      FundingRate(
        ...
        if (premium := entry.get('premium')) is not None
        else None,
      )
      for entry in chunk
    ]
...
async def funding_payments(
  self: PerpMarketMixin, start: datetime, end: datetime
