...
class SpotMarket(MarketMixin, Market):
  ...
  def candles(
    ...
    raise NotImplementedError(
      f"candles is not implemented for this market [{self.id}]: Kraken's OHLC "
      'endpoint serves only the 720 most recent candles per interval, whatever '
      "`since` says, so there is no walk to older history, and typed_kraken's "
      '`OhlcResult` declares the polling cursor but not the candle rows.'
    )
