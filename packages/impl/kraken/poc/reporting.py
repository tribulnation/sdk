...
# 3. **Timestamps.** `LedgerEntry.time` renders `TimestampSeconds` since the codegen fix, so it satisfies `BaseObservation.time`'s `AwareDatetime` directly and nothing is converted locally.
#
