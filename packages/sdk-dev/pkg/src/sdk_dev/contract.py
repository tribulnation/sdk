...
def render_method(
  method: ContractMethod, *, universe: list[str], catalogue: Any
) -> dict[str, dict]:
  """
  ...
  Returns:
    `{subset_key: {call, result, catalogueCall?, catalogueResult?}}` — `subset_key` is
    the selected venues' slugs, sorted and comma-joined, so the frontend can compute the
    same key from whatever it has selected without needing to know this function's
    internal venue ordering. `result` is a list of reveal chunks (for the wizard's
    staggered "run example" reveal); `catalogueResult`, like the primary `call`, is a
    single rendered string — the catalogue block is shown as one static block, never
    staggered.
  ...
  for size in range(1, len(ordered) + 1):
    for combo in combinations(ordered, size):
      ...
      entry: dict[str, Any] = {
        'call': call_tpl.render(ctx).strip('\n'),
...
def render_contract_file(
  contract: ContractFile, *, universes: dict[str, list[str]], catalogue: Any
) -> dict:
  """
  ...
  Returns:
    `{component: {...}, methods: {name: {group, signature, description, public,
    accountVenues, venues, subsets}}}`, JSON-serializable as-is. `venues` is the same
    list as `universes[name]` (the picker's own options for that method);
    `accountVenues` is the method's own initial/default selection; `subsets` is
    `render_method`'s output.
  """
  return {
    'component': contract.component.model_dump(exclude_none=True),
    'methods': {
      name: {
        **method.model_dump(
          include={'group', 'signature', 'description', 'public'}, exclude_none=True
        ),
        'accountVenues': method.example.accountVenues,
        'venues': universes[name],
        'subsets': render_method(method, universe=universes[name], catalogue=catalogue),
      }
