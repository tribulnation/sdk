...
from ..accounts import selected_accounts, require_credentials
...
async def collect(sdk: MarketSDK, account_id: str, public: bool, results: Results):
  ...
  async with venue:
    ...
    await results.attempt(
      'spot.candles',
      lambda: pages(spot_market.candles('1h', CANDLES_END - 72 * HOUR, CANDLES_END)),
    )
    await results.attempt(
      'spot.candles.straddle',
      lambda: pages(
        spot_market.candles(
          '1h', CANDLES_END - (SPOT_CANDLES_PAGE + 50) * HOUR, CANDLES_END
        )
      ),
    )
...
def bitget(market_account: str, pytestconfig: pytest.Config) -> Results:
  ...
  loop_of(pytestconfig).run_until_complete(
    collect(sdk, market_account, account.public, results)
  )
  return results
...
def check_spot_candles(result: list[Sequence[Candle]], *, count: int):
  """Check spot candle bounds, alignment and uniqueness without imposing ordering."""
  candles = [c for page in result for c in page]
  assert 0 < len(candles) <= count
  first = CANDLES_END - count * HOUR
  opens = [c.time for c in candles]
  assert len(set(opens)) == len(opens)
  assert all(time.utcoffset() is not None for time in opens)
  assert all(first <= time < CANDLES_END for time in opens)
  assert all((time - first) % HOUR == HOUR * 0 for time in opens)
  for page in result:
